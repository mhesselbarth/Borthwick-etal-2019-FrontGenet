...
#sites <- ralu.site
##Lots going on here because I can't find our data that corresponds, so:
sites<-readOGR("c:/users/richard/downloads/SSR_17_sites.shp")#this is location data, but it lacks any of the surface metrics at each point...where's that data? would really like suitability score that I can invert.
...
head(Raster$T.v_habitat_surface)
cost<-1/cost
cellStats(Raster$T.v_habitat_surface,stat = )
